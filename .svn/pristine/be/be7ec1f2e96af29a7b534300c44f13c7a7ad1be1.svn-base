...
"""
class MTianHuProcessor(MProcessor):
    def __init__(self,tableConfig):
        super(MTianHuProcessor, self).__init__(tableConfig)
    ...
    def setState(self, state):
        ftlog.debug('MTianHuProcessor setState:', state)
        self.__state = state
        
    def getState(self):
        if self.state != MTableState.TABLE_STATE_NEXT:
            ftlog.info('MTianHuProcessor.getState return:', self.state)
        return self.__state
    ...
    def initProcessor(self, actionID, seatId, state, tile, extendInfo, timeOut = 9):
        ...
        """
        ftlog.debug('MTianHuProcessor.initProcessor actionID', actionID
                , ' state:', state
                , ' seatId:', seatId
    ...
    def updateProcessor(self, actionID, state, seatId):
        ...
        if actionID != self.actionID:
            ftlog.debug('MTianHuProcessor.updateProcessor actionId error...')
            return False
        
        if seatId != self.__seatId:
            return False
        
        ftlog.debug('MTianHuProcessor.updateProcessor actionId:', actionID
                    , ' state:', state
                    , ' seatId:', seatId)
