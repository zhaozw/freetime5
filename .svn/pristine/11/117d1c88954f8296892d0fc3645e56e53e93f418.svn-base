...
class MTableTileMudanjiang(MTableTile):
    
    def __init__(self, playerCount, playMode, runMode):
        super(MTableTileMudanjiang, self).__init__(playerCount, playMode, runMode)
        # 宝牌
        self.__magic_tile = None
        
    def reset(self):
        """重置"""
        super(MTableTileMudanjiang, self).reset()
        self.__magic_tile = None
    ...
    def shuffle(self, goodPointCount, handTileCount):
        ...
        添加特殊逻辑，宝牌
        """
        super(MTableTileMudanjiang, self).shuffle(goodPointCount, handTileCount)
        # 选出最后一张当癞子
        self.__magic_tile = self.tiles.pop(-1)
        self.addSpecialTile(self.__magic_tile)
        ftlog.debug( 'MTableTileMudanjiang.shuffle changed tile:', self.__magic_tile )
    ...
    def getTingGangMode(self):
        return MTableTile.AFTER_TING_HU_KOU