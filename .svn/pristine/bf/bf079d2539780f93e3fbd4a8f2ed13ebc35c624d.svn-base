...
class MDropCardStrategyJiPingHuHigh(MDropCardStrategy):
    """
    ...
    """
    def __init__(self):
        super(MDropCardStrategyJiPingHuHigh, self).__init__()
    ...
    def isChi(self, allTiles, chiTile, chiPatterns, leftTiles, tingResults, seatResponse, extend={}):
        ...
        if self.isGetBetterChanceToWin(allTiles,leftTiles,[],tingResults,tingRule):
            return True
        else:
            return False
    ...
    def isGetBetterChanceToWin(self, tiles_player_hand, tiles_left, magicTiles, tingResults, tingRule=None):
        '''
            是否有更大的几率胡牌
        '''
        ftlog.debug('isGetBetterChanceToWin '
        ...
        nCountTingResults = 0
        for ting in tingResults:
            nCountTingResults += ting[2]
        if nCountTingResults==0:
            return True

        nNowCountTingResults = 0
        if tingRule:
            ...
            canTing, newTingResults = tingRule.canTing(tiles_player_hand, tiles_left, tiles_player_hand[0], magicTiles)
            ftlog.debug('isGetBetterChanceToWin '
                        , ' canTing:', canTing
                        , ' newTingResults:', newTingResults
                        )
            if canTing:
                for tingResult in newTingResults:
                    winNodes = tingResult['winNodes']
                    for winNode in winNodes:
                        nNowCountTingResults += winNode['winTileCount']

            if nNowCountTingResults > nCountTingResults:
                return  True
            else:
                return False
        else:
