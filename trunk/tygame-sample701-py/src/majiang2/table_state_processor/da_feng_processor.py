...
class MDafengProcessor(MProcessor):
    def __init__(self,tableConfig):
        super(MDafengProcessor, self).__init__(tableConfig)
    ...
    def setState(self, state):
        ftlog.debug('MDafengProcessor setState:', state)
        self.__state = state
        
    def getState(self):
        if self.state != MTableState.TABLE_STATE_NEXT:
            ftlog.info('MDafengProcessor.getState return:', self.state)
        return self.__state
    ...
    def updateProcessor(self, actionID, state, seatId):
        ...
        if actionID != self.actionID:
            ftlog.debug('MDafengProcessor.updateProcessor actionId error...')
            return False
        
        if seatId != self.__seatId:
            return False
        
        ftlog.debug('MDafengProcessor.updateProcessor actionId:', actionID
                    , ' state:', state
                    , ' seatId:', seatId)
