...
"""
class MShuffleHuProcessor(MProcessor):
    def __init__(self,tableConfig):
        super(MShuffleHuProcessor, self).__init__(tableConfig)
    ...
    def setState(self, state):
        ftlog.debug('MShuffleHuProcessor setState:', state)
        self.__state = state
        
    def getState(self):
        if self.state != MTableState.TABLE_STATE_NEXT:
            ftlog.info('MShuffleHuProcessor.getState return:', self.state)
        return self.__state
    ...
    def initProcessor(self, actionID, seatId, state, tile, extendInfo, timeOut = 9):
        ...
        """
        ftlog.debug('MShuffleHuProcessor.initProcessor actionID', actionID
                , ' state:', state
                , ' seatId:', seatId
    ...
    def updateProcessor(self, actionID, state, seatId):
        ...
        if actionID != self.actionID:
            ftlog.debug('MShuffleHuProcessor.updateProcessor actionId error...')
            return False
        
        if seatId != self.__seatId:
            return False
        
        ftlog.debug('MShuffleHuProcessor.updateProcessor actionId:', actionID
                    , ' state:', state
                    , ' seatId:', seatId)
