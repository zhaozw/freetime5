...
class TYRoom(tygame.TYRoom):
    ...
    def matchCasualGamePlayer(self, userIdA, userIdB):
        if _DEBUG:
            debug("In matchCasualGamePlayer @@@@@  userid_A = ", userIdA, "  userId_B = ", userIdB)
        #todo need pass roomId or not
        pluginCross.srvtable.doCreateNewTable(userIdA, userIdB)
